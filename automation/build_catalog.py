...
def load_backlog() -> dict[str, dict[str, str]]:
    ...
    for idea in raw.get("ideas", []):
        ...
        if period:
            by_period[str(period)] = {key: str(value) for key, value in idea.items()}
    return by_period
...
def render_catalog(products: list[dict[str, str]]) -> str:
    cards = []
    for item in products:
        cards.append(
            """<article class="card"><div class="meta"><span>{period}</span><span>{category}</span></div>"
            "<h2>{title}</h2><p>{pitch}</p><a class="open" href="{url}">فتح المنتج</a></article>""".format(
                period=html.escape(item["period"]),
                category=html.escape(item["category"]),
                title=html.escape(item["title"]),
                pitch=html.escape(item["pitch"]),
